export const GET_DEUDAS_SIIM_SQL = (
  ...
  return `
    WITH 
-- BLOQUEO GLOBAL: Identifica todos los clientes que tienen deudas actualmente en coactiva activa
clientes_en_coactiva AS (
    SELECT DISTINCT f."idPropietarioEmision" AS id_cliente
    FROM coactiva_factura cf
    JOIN coactiva ca ON ca.id = cf.id_coactiva
    JOIN factura f ON f.id = cf.id_factura
    WHERE ca.estado = 1
)
...
export const GET_DEUDAS_SIIM_SQL_FILTROCOACTICA_ESPECIFICO = (
  fechaStr: string,
  modulos: { urbano: number; rural: number; agua: number }
) => {
  return `
